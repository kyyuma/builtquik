...
function Login($con)
{
	if(!empty($_POST['lemail']))
	{
		...
		$hash = mysqli_query($con,"SELECT password FROM customer WHERE email = '$_POST[lemail]'");
		...
		if(password_verify($_POST['lpass'], $hashResult)){
			$query = mysqli_query($con,"SELECT * FROM customer WHERE email = '$_POST[lemail]' ") /*AND active = '1'"*/ or die(mysql_error());
